...
with col_yasin:
    ...
    if selected_yasin_donem:
        ...
        for idx, row in df_display_yasin.iterrows():
            ...
            btn_color = "#22c55e" if durum == "Evet" else "#fca5a5"  # Evet: yeşil, Hayır: açık kırmızı

            btn_key = f"yasin_{selected_yasin_donem}_{row['Cüz No']}"
            if col3.button(btn_label, key=btn_key, help="Durumu değiştir"):
                    ...
                    if donem["Hatim Dönem"] == selected_yasin_donem:
                        for j, cuz in enumerate(donem["Cüzler"]):
                            if cuz["Cüz No"] == row["Cüz No"]:
                                ...
                                save_data(all_data)
                                st.rerun()
            # Sadece bu butonu hedefleyen CSS:
            col3.markdown(
                f"""
                <style>
                [data-testid="stButton"][data-baseweb="button"] button[kind="secondary"]#{btn_key} {{
...
with col_cuz:
    ...
    if selected_cuz_donem:
        ...
        for idx, row in df_display_cuz.iterrows():
            ...
            durum = row["Okunma Durumu"]
            btn_label = "Evet" if durum == "Evet" else "Hayır"
            btn_color = "#22c55e" if durum == "Evet" else "#3b82f6"  # Evet: yeşil, Hayır: mavi
            ...
            col3.markdown(
                f"""
                <style>
                div[data-testid="stButton"] > button[title="Durumu değiştir"] {{
                    background-color: {btn_color} !important;
                    color: white !important;
                    font-weight: bold;
                    border-radius: 5px;
